...
// Поиск идентификатора в таблице по имени
   ...
   bool isInCounter(const T&id_name) const
   {
        ...
        {
            for(auto it=this->m_hash_table[pos].begin();it!=this->m_hash_table[pos].end();it++)
            {
   ...
   hashCounter operator || (hashCounter<T> &right)
    {
       hashCounter<T> newhs;
        for (int i=0;i<this->capacity;i++)
        {
            std::list<Identifier<T>>& line = m_hash_table[i];
            typename std::list<Identifier<T>>::iterator it;
                for(it=this->m_hash_table[i].begin();it!=this->m_hash_table[i].end();it++)
                {
                    if(!(line.empty()))
                    {
                    T path =(*it).getName();
                ...
                for(it=right.m_hash_table[i].begin();it!=right.m_hash_table[i].end();it++)
                {
   ...
   void operator >>(std::string fileName)
   {
        ...
        for (int i=0;i<int(this->m_hash_table.capacity());i++)
        {
         ...
         {
            typename std::list<Identifier<T>>::iterator it;
            for(auto it=this->m_hash_table[i].begin();it!=this->m_hash_table[i].end();it++)
            {
